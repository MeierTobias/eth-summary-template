% set document margins
\if 1\addheaderfooter
    % margins with header and footer
    \newcommand{\customtopmargin}{20mm}
    \newcommand{\custombottommargin}{0mm}
\else
    % margins without header and footer
    \newcommand{\customtopmargin}{7mm}
    \newcommand{\custombottommargin}{7mm}
\fi

% document setup
\usepackage[left=7mm,right=7mm,top=\customtopmargin,bottom=\custombottommargin,\orientationmode,\papersize]{geometry}
\usepackage{multicol,multirow}
\usepackage{inputenc}

% for argument parsing
\usepackage{xparse}

% enable colors
\usepackage{xcolor,color} % standard colors (blue, red, etc.https://www.namsu.de/Extra/pakete/Xcolor.html )
\def\columnseprulecolor{\color{lightgray}}
% language
\usepackage[english]{babel}

% for images
\usepackage{graphicx}
\usepackage{graphbox}
\graphicspath{ {\imagePath} }

% Images combined with texts
\usepackage{wrapfig}

% fonts
\usepackage{fontspec}
% code font
\setmonofont{custom_mono}[
    % Scale=MatchLowercase,
    Scale=0.96,
    Path=\templatePath/fonts/,
    Extension=.ttf,
    UprightFont=SourceCodePro-Regular,
    ItalicFont=IBMPlexMono-Italic,
    BoldFont=SourceCodePro-SemiBold,
]

% more fine control for lists
\usepackage{enumitem}
\setlist[itemize]{leftmargin = 12pt, label = ·}
\setlist[enumerate]{leftmargin = 14pt}
\setlist{noitemsep}
%\setlist{nosep}

% multiple line comments and testing text
\usepackage{comment} % \begin{comment} \end{comment} 
\usepackage{blindtext} % inserts lorem ipsum like text

% table that supports equations
\usepackage{tabularx}
\usepackage{booktabs}
\usepackage{rotating}

% algorithm environment
\usepackage{algorithm, setspace}
\usepackage[noend]{algpseudocode}

% diagonal strikethrough
\usepackage{cancel}

% bibliography
\usepackage[natbib=true, style=ieee, sorting=none]{biblatex}
\usepackage{csquotes}

% set the section numbering down to the paragraph level
\setcounter{secnumdepth}{4}
\setcounter{tocdepth}{4}

% column separation
\providecommand{\newcol}{\vfill\null\columnbreak}

% paragraph separation
\providecommand{\newpar}{\par\vspace*{4pt}}